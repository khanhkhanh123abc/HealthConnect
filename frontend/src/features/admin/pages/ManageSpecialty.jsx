...
import {
    createNewSpecialtyService, getAllSpecialty,
...
const ManageSpecialty = () => {
    ...
    const handleSubmit = async () => {
        ...
        const res = editingId
            ? await updateSpecialtyService({ ...form, id: editingId })
            : await createNewSpecialtyService(form);
        const ok = res?.errCode === 0 || res?.data?.errCode === 0;
        if (ok) {
            toast.success(editingId ? 'Specialty updated successfully.' : 'Specialty added successfully.');
    ...
    const handleEdit = (spec) => {
        ...
        setForm({
            ...
            descriptionHTML: spec.descriptionHTML || '', descriptionMarkdown: spec.descriptionMarkdown || ''
        });
        setPreviewImgURL(spec.image || '');
        window.scrollTo({ top: 0, behavior: 'smooth' });
        toast.info(`Editing: ${spec.name}`);
    };

    const handleDelete = async (spec) => {
        if (!window.confirm(`Are you sure you want to delete "${spec.name}"?`)) return;
        const res = await deleteSpecialtyService(spec.id);
        const ok = res?.errCode === 0 || res?.data?.errCode === 0;
        if (ok) { toast.success('Specialty deleted.'); fetchSpecialties(); }
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-semibold text-gray-900">Specialty Management</h1>
                    <p className="text-sm text-gray-500 mt-1">
                        {editingId
                            ? <span className="text-amber-600 font-medium">Editing specialty</span>
                            : `${specialtyList.length} specialties in the system`}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200/60 p-6 mb-6 space-y-5">
                <h2 className="text-sm font-semibold text-gray-900">
                    {editingId ? 'Edit Information' : 'Add New Specialty'}
                </h2>
                ...
                <div className="grid grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" value={form.name}
                            ...
                            className={INPUT} placeholder="Orthopedics" />
                    </div>
                    <div>
                ...
                <div>
                    <label className={LABEL + ' mb-2'}>Introduction Article</label>
            ...
            <div className="bg-white rounded-2xl border border-gray-200/60 overflow-hidden">
                ...
                {isLoadingList ? (
                    ...
                    <div className="text-center py-12 text-sm text-gray-400">No specialties found</div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-6">
                        {filteredList.map((spec, idx) => (
                            <div key={spec.id}
                                className={`flex items-center gap-4 p-4 rounded-2xl border transition-shadow hover:shadow-md hover:shadow-gray-200/50 duration-300 ${editingId === spec.id ? 'border-amber-300 bg-amber-50' : 'border-gray-200/60 bg-white'}`}>
                                <img src={spec.image || ''}
                                    alt={spec.name}
                                    className="w-14 h-14 object-cover rounded-xl border border-gray-200 flex-shrink-0" />
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-gray-900 truncate">{spec.name}</p>
                                    <p className="text-xs text-gray-400 mt-0.5">#{idx + 1}</p>
                                </div>
